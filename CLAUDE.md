...
`App.svelte` owns file intake (drag & drop, `getInitialFile`, `onOpenFile`) and the welcome screen; it hands a `videoSrc` URL to `VideoPlayer.svelte`, which owns all playback state (`paused`, `duration`, `currentTime`, `volume`, `userPlaybackRate`) via `bind:` on the `<video>` element. `VideoControls.svelte` binds nothing back except `volume` — it renders `displayTime`, reports seeks through an `onSeek(time)` callback (so the progress bar goes through the same queue as every other gesture) and reports scrubbing through `onScrubStart`/`onScrubEnd`.

Input logic lives in `src/lib/logic/` as **plain classes constructed with a `() => videoElement` getter plus a callback context object** — not stores, not components. `KeyboardHandler`, `TouchpadHandler`, `DragSeekHandler`, `HoldZoneHandler` and `ClickSeekHandler` never touch Svelte state directly; they call back into `VideoPlayer.svelte`. The getter indirection exists because `videoElement` is undefined when the handlers are constructed. Presentational components in `src/lib/components/` are pure props-in.

Three small classes own state rather than input and are constructed with no arguments: `SeekIndicator` (`seek-indicator.svelte.ts`), `ControlsVisibility` (`controls-visibility.svelte.ts`) and `SeekQueue`. They hold `$state` fields, so `VideoPlayer.svelte` reads them through getters (`controls.isVisible`, `seekIndicator.delta`) instead of keeping mirrored local variables.

Values that must agree across modules live in `src/lib/logic/constants.ts` (`SEEK_SECONDS_PER_PIXEL`, `GESTURE_MOVE_THRESHOLD_PX`, `HOLD_DELAY_MS`, `CLICK_SEEK_SECONDS`); a divergence there breaks gesture arbitration silently. Values local to one handler (rewind step, recovery timings, autohide delay) stay in their own file.
...
- **The long actions themselves live in `HoldActionRunner`** (`src/lib/logic/hold-actions.ts`), not in the keyboard handler: `rewind` / `boost` (×2) / `forward` (×16 + warp). `VideoPlayer.svelte` owns exactly one runner and injects it into both `KeyboardHandler` and `HoldZoneHandler`, so a second hold folds the first one down (`start()` calls `stop()`) and there is a single place that restores `userPlaybackRate`, clears the rewind interval, and re-pauses a video that was only playing for ×16.
- **Hold-by-zone with the mouse** (`src/lib/logic/hold-zones.svelte.ts`): the frame splits into three vertical thirds, and holding the left button for 200 ms in one of them runs the same action as holding ←, Space, or → respectively. The left button serves three gestures, and the arbitration between them lives in **`src/lib/logic/pointer-router.ts`** — that class is the only place the rules are written, and `VideoPlayer.svelte` just forwards raw pointer events to it: under `HOLD_DELAY_MS` it is still a click (play/pause, double-click ±10 s); moving past `GESTURE_MOVE_THRESHOLD_PX` before the timer fires hands the gesture to drag-seek (`cancelPending()`, same constant on both sides); once a hold is live the router stops forwarding `pointermove` to `DragSeekHandler` so a drag can no longer start. Both handlers get their own one-shot `shouldSuppressClick()`, and the router's `shouldSuppressClick()` must call *both* without short-circuiting (each call consumes its own flag) or a hold would end in a pause. The self-heal paths are the same as drag-seek's: `buttons === 0` on move, `pointercancel`, `lostpointercapture`, window `blur`.
...
- **Every seek goes through the one shared `SeekQueue`** (`src/lib/logic/seek-queue.svelte.ts`) that `VideoPlayer.svelte` owns and injects into all four paths — keyboard, touchpad, drag, and the progress bar (`onSeek`), plus double-click seeking. Nothing outside that module may assign `videoElement.currentTime`; a `grep -rn '\.currentTime = ' src/` should only ever hit `seek-queue.svelte.ts`. The queue hands the element one seek at a time, coalescing whatever arrives while a seek is in flight and flushing it on `seeked`. Assigning per event is what made a fast gesture freeze on a `file://` source: each assignment cancelled the in-flight seek, so almost nothing completed (measured: 3 of 40 seeks and 3 painted frames, against 30 of 30 through the queue), and an element that starts out already `seeking` can stay wedged for good. Two rules keep it correct: relative steps must accumulate from `getTargetTime()`, never from `currentTime` (the element lags behind the queue, so steps get dropped), and `handleSeeked()` must clear the target once the element arrives (a stale target would show through on the next seek and make the progress marker flip between two positions).
- **`displayTime` vs `currentTime`**: the progress bar renders `displayTime`, sampled per `requestAnimationFrame` from `seekQueue.getTargetTime()` while playing or seeking — `timeupdate`, which drives `currentTime`, only fires ~4×/s and made the marker step. The rAF loop is gated to playing/gesture states (`controls.isPinned` doubles as "a gesture is in progress"); when idle-paused, `displayTime` mirrors `currentTime` — but only while `seekQueue.isSeekPending()` is false. A paused seek (click on the bar, ←/→) finishes long after the gesture ends, and the bound `currentTime` only catches up on `timeupdate`, so falling back to it right away threw the marker forward, back, and forward again. That check is why the queue's `pending`/`target` are `$state` and the module is `.svelte.ts`.
- **Drag-to-seek** (`src/lib/logic/drag-seek.svelte.ts`): left button held anywhere on the video and moved, `SEEK_SECONDS_PER_PIXEL` per pixel of horizontal movement — the same constant `TouchpadHandler` uses. `GESTURE_MOVE_THRESHOLD_PX` separates it from a click; below it the gesture is still a click. It pauses the video for the duration of the drag and resumes on release if it was playing, and Escape/`pointercancel` restore the starting position. `click` always fires after `pointerup`, so `shouldSuppressClick()` eats exactly one click per completed drag — without it every drag would end in a pause. `pointerdown` is bound to the `<video>` element, `pointermove`/`pointerup` to `window` (plus `setPointerCapture`), so the gesture survives the cursor leaving the window. A lost `pointerup` would wedge the player — the video stays paused and every mouse move keeps dragging `currentTime` — so the gesture self-heals: `handlePointerMove` ends it when `e.buttons === 0`, and `lostpointercapture`/window `blur` call `handleInterrupt()`.
- **Decoder crashes are recovered from, not ignored** (`src/lib/logic/recovery.ts`): fast seeking can kill the hardware decoder (on macOS `MEDIA_ERR_DECODE` / `VTDecompressionOutputCallback -12909`). The element then drops to `readyState 1` and stays `seeking === true` forever — `seeked` never fires, the `SeekQueue` keeps waiting for it, and **every** gesture dies at once. The only cure is `video.load()`, so `PlaybackRecovery` reloads the source and restores position/rate/play-state; it also has a watchdog for a seek that never completes (`STUCK_SEEK_MS`) and a cap of three reloads per minute so a file that dies at the same frame can't loop. Two things it must keep doing: `seekQueue.reset()` (the pending seek belongs to a dead element) and suppressing the window resize on the second `loadedmetadata` (`recovery.isActive()` in `handleLoadedMetadata`), or the window re-centers on every recovery.
- **Dev-only event log** (`src/lib/logic/debug-log.ts`): a ring buffer of the last 5000 events (gestures, queue transitions, `<video>` events) with a state snapshot per entry. `Ctrl/Cmd+Shift+D` writes it to `debug-logs/*.jsonl` via the `write-debug-log` IPC channel, which `main.js` registers **only** when `NODE_ENV === 'development'`; `PlaybackRecovery` dumps automatically when it fires. `window.playerDebug` exposes `dump()/entries()/clear()/mirror()` in DevTools. Everything is a no-op in production (`import.meta.env.DEV`), so instrumenting a new gesture with `debugLog.event()` costs nothing there.
- **Click handling** (`src/lib/logic/click-seek.ts`) is YouTube-style: `event.detail === 1` schedules play/pause after 220 ms; even counts in the series seek ±`CLICK_SEEK_SECONDS` by screen half and cancel the pending toggle. Drag-seek's `onSeekStart` and hold-zone's `onHoldStart` must call `cancelPendingToggle()`, or a "click then hold" series ends in a pause.

- **The seek indicator is one component fed by four sources** (`src/lib/logic/seek-indicator.svelte.ts`): drag, touchpad swipe, double-click and held ←/→ (`boost` is not a seek and must not show it). Two rules: the numbers are never cleared when a gesture ends — the indicator fades over 150 ms and zeroes would be visible as a flicker — and a source writes its numbers *before* it turns the indicator on, or the appearing frame shows the previous gesture's values. Hold has no step of its own: `VideoPlayer.svelte` feeds it `displayTime` through `trackHold()` each frame, guarded by `isHolding` so the effect does not re-run while merely playing.

- **The controls panel has two separate "don't hide" reasons** (`src/lib/logic/controls-visibility.svelte.ts`): `pin()`/`unpin()` for a gesture in progress (progress-bar scrub, drag-seek, held ←/→ or an edge zone — the mouse is not moving, so without pinning the bar would vanish mid-seek) and `hoverStart()`/`hoverEnd()` for the cursor resting on the panel. `keepAlive()` is the "user did something" call; it shows the panel but arms the autohide timer only when neither reason is active. `show()` is for states rather than actions (video ended, recovery in progress).
