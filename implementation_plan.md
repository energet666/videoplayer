...
│           ├── seek-queue.svelte.ts # Общая очередь перемоток (одна на <video>)
...
- [x] Общая очередь seek-ов (`seek-queue.svelte.ts`) — одна на элемент, для всех путей
...
- [x] Кадр поделён по горизонтали на три равные зоны; удержание левой кнопки
  ...
  - Центр → как зажатый пробел (×2)
  - Правая треть → как зажатая `→` (×16 + warp-эффект, на паузе видео
    запускается временно)
